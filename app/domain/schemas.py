...
class TaskStatusResp(BaseModel):
    ...
    exp: Optional[int] = Field(None, description="Expiration timestamp for signed URL (completed only)")
    sig: Optional[str] = Field(None, description="Signature for file download (completed only)")
